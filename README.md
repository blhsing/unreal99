...
依原作規則實作：**踩上控制點即完成佔領**，沒有讀條、不需要持續站立，也不需要清場。之後
**每持有一個控制點，每五秒為該隊得一分**，所以持有兩點的得分速度正好是一點的兩倍——這條乘法
就是整個模式的經濟。先達得分上限（預設 100）或時間結束時分數最高的隊伍獲勝。原作的 DOM 地圖
一律是三個控制點，本作四張也都是三個。

HUD 下方會列出每個控制點的名稱與目前歸屬（原作為每個點命名，熔鉛廠的三點就叫 Tower、Bridge、
Storage），剛易主的點會閃動；控制點本身在場中以所屬隊伍的顏色發光，跨房間也看得出歸屬。

### 電腦對手的戰術

電腦對手不是在控制點附近打死亡競賽，而是真的會去搶點：

* 己方未持有的點才是目標；中立點的權重比敵方持有的點低（×0.7），因此同樣距離下優先搶中立點，
  但敵方點夠近時照樣去拔。
* 隊內依編號分工，避免整隊擠向同一個點而讓其餘兩點無人問津。
* **只有在領先且持有兩點以上時才會分出人手防守**；落後或平手時全隊進攻——守著劣勢只是輸得慢一點。
* 進攻者把控制點當作目標點位（objective）承諾路線，防守者則以一般戰鬥行為在點外圍游走，
  這樣才會閃避、才不會站在點上被打。
