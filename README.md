...
| `--vehiclecontrol` | 無視窗驗證轉向會帶動視角、座位切換、每個武裝座位都真的能開火、座艙與車體模型密度，以及所有地圖上的載具都停在走得到的地方 |
...
<td><br><b>切換提示</b><br>HUD 固定為十一個武器槽，不會因跨世代武器增加而一直縮小。數字鍵 1～9 選擇前九槽；0 單擊選擇救世主／指示器槽，快速連按兩次 0 選擇狙擊／閃電槍槽。同一快捷鍵再次按下會在該槽已持有且有彈藥的同類武器間切換；Q／E 或滑鼠滾輪仍可循環全武器。</td>
</tr>
...
| 武器槽快捷 | 數字鍵 `1`～`0`（再次按下切換同槽武器；`0` 連按兩次選狙擊槽） | 可自行指定 | 可自行指定 |
| 計分板 | `Tab` | `Delete` | `B` |
| **上下載具** | `F` | `Enter` | `K` |
| **切換座位** | `C` | `Num -` | `L` |
