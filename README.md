...
| `--mapstats` | 無視窗列出三十六張地圖各自的靜態三角形數，低於 6,000 者視為幾何量不足並失敗 |
...
<td><br><b>切換提示</b><br>HUD 只列出目前地圖實際提供的武器家族，空槽不再佔位；因此同一把武器在不同地圖可能使用不同數字鍵。玩家一預設以 1～9 選擇前九槽、0 單擊選第十槽，快速連按兩次 0 選第十一槽。同一快捷鍵再次按下會在該槽已持有且有彈藥的同類武器間切換；Q／E 或滑鼠滾輪仍可循環全武器。</td>
</tr>
...
| 武器槽快捷 | 數字鍵 `1`～`0`（依目前地圖緊密排列；`0` 連按兩次選第 11 槽） | 可自行指定 | 可自行指定 |
| 計分板 | `Tab` | `Delete` | `B` |
| **上下載具** | `F` | `Enter` | `K` |
| **切換座位** | `C` | `Num -` | `L` |
