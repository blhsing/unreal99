...
十七座競技場，**每一座都是向 1999 年原作經典地圖的致敬之作**——佈局、路線與節奏取自記憶中的
原版，幾何、材質與道具配置則完全用 `LevelBuilder` 重新打造，沒有反編譯、轉檔或沿用任何原作
關卡資料。
...
| `--map` | 名稱 | 致敬對象 | 特色 |
| --- | --- | --- | --- |
| 0 | DM-莫比亞斯 | *Morbias ][* | 一個圓、一根柱子，武器只有兩把。最純粹的近身混戰 |
...
`Esc` 暫停選單　　`F3` 效能資訊　　`F5` 快速儲存　　`F9` 快速載入　　`F11` 切換全螢幕　　`F12` 螢幕截圖
...
```
%APPDATA%\Unreal99\
  settings.json     畫質、視野、靈敏度、音量、四組按鍵配置、裝置指派、上次的對戰設定
...
上一次使用的對戰設定（競技場、模式、人數、電腦數量與難度、各項上限）也會一併記住。
...
代打電腦的程度是**獨立設定**的，與對手難度分開——可以讓一個新手代打去打一群神級對手，
或反過來。`--demo` 命令列參數效果相同。
