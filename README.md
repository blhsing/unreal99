...
* Remote Desktop 工作階段不使用會把遠端絕對座標反覆置中的 Raw 游標擷取；遊戲保留隱藏但不
  置中的游標，並從共用 Raw Input 封包讀取位移、按鍵與滾輪，因此按移動鍵不會令視角突然抬高
  或旋轉。RDP 連上或離開時會即時切換路由。
...
取回原本的玩家欄位，而新滑鼠第一次移動後會自動補上空著的自動指派欄位，不必重啟遊戲。
