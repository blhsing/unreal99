...
foreach ($mapId in $MapIds) {
    if ($mapId -lt 0 -or $mapId -gt 35) { throw "Map id is outside 0..35: $mapId" }
}
...
try {
    foreach ($mapId in $MapIds) {
        ...
        $mode = if ($mapId -ge 34) { 9 }
                elseif ($mapId -ge 28) { 8 }
                elseif ($mapId -ge 25) { 7 }
...
$results | Select-Object MapId, Map, Mode, Passed, ActiveSeconds, TravelMeters,
    ...
    ShotsFired, ShotsHit, AccuracyPercent, MaxAccuracyPercent,
    ...
    DrowningDeaths, JumpPadLaunches, RepeatedJumpPadLaunches,
